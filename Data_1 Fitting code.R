...
library(VGAM)
## Data -----------------------
x=c(1,3,3,6,7,7,10,12,14,15,18,19, 
...
ml.bdme <- maxLik(loglik.bdme, start=c(.5), method="NR")
...
# MLE
ml.dlog <- maxLik(loglik.dlog, start=c(0.5, mean(x)), method="NR")
...
# MLE
ml.dburr <- maxLik(loglik.dburr, start=c(0.996,93.0019), method="CG")
...
# MLE
ml.dg <- maxLik(loglik.dg, start=c(1, 12), method="NR")
...
# MLE
ml.edl <- maxLik(loglik.edl, start=c(0.5, 1), method="NR")
...
# MLE
ml.eg <- maxLik(loglik.eg, start=c(0.5, 1), method="NR")
...
# MLE
ml.tdl <- maxLik(loglik.tdl, start=c(0.5, 1), method="NR")

p.tdl    <- ml.tdl$estimate[1]
beta.tdl <- ml.tdl$estimate[2]
se.tdl   <- sqrt(diag(vcov(ml.tdl)))
...
# MLE
ml.dsgli <- maxLik(loglik.dsgli, start = c(0.5, 1), method = "NR")
